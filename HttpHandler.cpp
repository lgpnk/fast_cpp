    ...
    switch(arg->mod)
    {
	  ...
	  while(ret_value > 0) 
	  {
		...
		len += sprintf(my_string_corner + len, " ");
	      ...
	      len+= sprintf(my_string_corner + len, "%sEOK", CaptureHandler::get_strkmeans());
	      len+= sprintf(my_string_corner + len, "0;%d;%d;%d;%d;%d;EOT", CaptureHandler::get_profiler().num_corner, CaptureHandler::get_profiler().m_time_corner, CaptureHandler::get_profiler().m_time_supp, CaptureHandler::get_profiler().m_time_kmeans, time_pkg);
		      ...
		      startT = time(NULL);
		      time_pkg += (((startT - endT)*1000000 )- time_pkg) / 200;
		      endT = startT;
	  ...
	  while(ret_value > 0) 
	  {
		...
		len += sprintf(my_string_edge + len, " ");
	      ...
	      len+= sprintf(my_string_edge + len, "0;%d;%d;EOT", CaptureHandler::get_profiler().m_time_edge, CaptureHandler::get_profiler().m_time_proc);
		      ret_value = net_http_send_multipart_content(fd,
							  HTTP_TIMEOUT,
							  txt_text_html_utf8,
							  my_string_edge,
	  ...
	  while(ret_value > 0) 
	  {
		...
		len += sprintf(my_string_ec + len, " ");
	      ...
	      len += sprintf(my_string_ec + len, "EOF");
	        len+= sprintf(my_string_ec + len, "0;%d;%d;%d;%d;%d;%d;EOT", CaptureHandler::get_profiler().m_time_grab, CaptureHandler::get_profiler().m_time_corner, CaptureHandler::get_profiler().m_time_supp, CaptureHandler::get_profiler().m_time_edge, CaptureHandler::get_profiler().m_time_kmeans, time_pkg);
		      ret_value = net_http_send_multipart_content(fd,
							  HTTP_TIMEOUT,
							  txt_text_html_utf8,
							  my_string_ec,
